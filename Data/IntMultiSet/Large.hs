...
-----------------------------------------------------------------------------
-- |
-- Module      :  Data.IntMultiSet.Large
-- Copyright   :  (c) Twan van Laarhoven 2008, Masahiro Sakai 2013
-- License     :  BSD-style
...
import Data.MultiSet.Large (MultiSet)
import qualified Data.IntMap.Strict as Map
import qualified Data.IntSet as Set
import qualified Data.List as List
import qualified Data.MultiSet.Large as MultiSet
...
instance Data IntMultiSet where
  ...
  dataTypeOf _   = mkNoRepType "Data.IntMultiSet.Large.IntMultiSet"
...
deleteN :: Integer -> Integer -> Maybe Integer
deleteN n m
  | m <= n    = Nothing
  | otherwise = Just (m - n)
...
minKey :: IntMap a -> Int
minKey = maybe (error "IntMultiSet.Large.findMin: empty multiset") (fst . fst) . Map.minViewWithKey

-- | /O(log n)/. The maximal key of an IntMap.
maxKey :: IntMap a -> Int
maxKey = maybe (error "IntMultiSet.Large.findMax: empty multiset") (fst . fst) . Map.maxViewWithKey
...
maxView x
  | null x    = fail "IntMultiSet.Large.maxView: empty multiset"
  | otherwise = return (deleteFindMin x)
...
-- /The precondition (input list is strictly ascending) is not checked./
fromDistinctAscOccurList :: [(Int,Occur)] -> IntMultiSet 
...
fromOccurMap :: IntMap Occur -> IntMultiSet
fromOccurMap = MS
...
splitOccur :: Int -> IntMultiSet -> (IntMultiSet,Occur,IntMultiSet)
